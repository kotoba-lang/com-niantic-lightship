(ns niantic_lightship.main
  "Kotodama WASM entrypoint for the Niantic_lightship clean-room actor (L4) — Clojure port.
  ...
  facts (`niantic_lightship.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "niantic_lightship")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"niantic_lightship.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "niantic_lightship-compat" :tier tier :entities entities} 200])
...
(defn start! [] :niantic_lightship-compat/ready)
